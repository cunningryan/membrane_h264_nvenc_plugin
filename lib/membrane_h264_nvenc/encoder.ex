defmodule Membrane.H264.Nvenc.Encoder do
  @moduledoc """
  Membrane element that encodes raw video frames to H264 format.

  Uses NVENC (NVIDIA hardware encoder) when available, falls back to x264 (software) otherwise.
  This allows the same code to run on GPU-equipped servers (using hardware encoding) and
  local development machines without NVIDIA GPUs (using software encoding).
  ...
  alias Membrane.H264
  alias Membrane.H264.Nvenc.Common
